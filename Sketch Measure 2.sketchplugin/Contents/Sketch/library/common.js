...
SM.extend({
    ...
    checkSlice: function(layer, layerData){
        if(layerData.type == "slice" || ( layerData.type == "symbol" && this.hasExportSizes(layer.symbolMaster()) && !this.symbolCache[layer.symbolMaster().objectID()]) ){
    ...
    checkSymbol: function(layer, layerData, layersData){
        if( layerData.type == "symbol" ){
            var self = this,
                layerObjectID = layerData.objectID,
                symbolObjectID = this.toJSString(layer.symbolMaster().objectID()),
                parentPoint = {x: layerData.rect.x, y: layerData.rect.y},
                symbolCache = this.symbolCache[symbolObjectID];

            layerData.objectID = symbolObjectID;

            if(!symbolCache){
                this.symbolCache[symbolObjectID] = {
                        layers: self.getLayers(layer.symbolMaster(), true),
                        hasSlice: self.hasExportSizes(layer.symbolMaster())
                    };
                symbolCache = this.symbolCache[symbolObjectID];
            }

            if( !symbolCache.hasSlice ){
                for (var i = 0; i < symbolCache.layers.length; i++) {
                    var symbollayerData = JSON.parse( JSON.stringify( symbolCache.layers[i] ) );
                    symbollayerData.objectID = symbollayerData.objectID;
                    symbollayerData.rect.x = parentPoint.x + symbollayerData.rect.x;
                    symbollayerData.rect.y = parentPoint.y + symbollayerData.rect.y;
                    layersData.push(symbollayerData);
                }
            }
            else if( this.sliceCache[symbolObjectID] ){
                layerData.exportable = this.sliceCache[symbolObjectID];
            }
        }
    },
    ...
    getLayers: function( artboard, isSymbol ){
        var self = this,
        ...
        while( layer = layers.nextObject() ) {
            ...
            if(layer && this.is(layer, MSLayerGroup) && /NOTE\#/.exec(layer.name())){
                ...
                notesData.push({
                    rect: this.rectToJSON(textLayer.absoluteRect(), artboardFrame),
            ...
            layerData.rect = this.rectToJSON(layer.absoluteRect(), artboardFrame);
            ...
            this.checkSymbol(layer, layerData, layersData);
        }

        if(isSymbol) return layersData;
