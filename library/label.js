var CreateLabel = function(layer, content, position, data, idname){
    ...
    var gX     = Math.round(frame.x + (frame.width - labelWidth) / 2),
        gY     = Math.round(frame.y + (frame.height - labelHeight) / 2),
        gapX   = Math.round((labelWidth - 8) / 2),
        gapY   = Math.round((labelHeight - 8) / 2);

    if( position == 'top' ) {
      gapY = labelHeight - 4;
      gY   = frame.y - labelHeight - 6;
    }
    else if( position == 'right' ) {
      gapX = -4;
      gX   = frame.x + frame.width + 6;
    }
    else if( position == 'bottom' ) {
      gapY = -4;
      gY   = frame.y + frame.height + 6;
    }
    else if( position == 'left' ) {
      gapX = labelWidth - 4;
      gX   = frame.x - labelWidth - 6;
    }

    setPosition(text, 5, 5);
    setPosition(gap, gapX, gapY);
    setPosition(group, gX, gY);
